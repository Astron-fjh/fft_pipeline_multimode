%% Description:
%   本代码实现了256点FFT和IFFT的定点化处理，并与标准FFT和IFFT进行对比。
...
%
%   This code implements the fixed-point processing of 256-point FFT and IFFT, and compares the results with standard FFT and IFFT.
...
% 打开文件并写入数据
fd_re = fopen("./256点/x_re256.txt", 'wb');
fd_im = fopen("./256点/x_im256.txt", 'wb');
for i = 1:N
    % 将16位有符号整数转为补码并保存
...
fd_re = fopen("./256点/x_fft256_re.txt", 'wb');
fd_im = fopen("./256点/x_fft256_im.txt", 'wb');
for i = 1:N
...
fd_re = fopen("./256点/x_sim_fft256_re.txt", 'wb');
fd_im = fopen("./256点/x_sim_fft256_im.txt", 'wb');
for i = 1:N
...
%%
% 读取RTL的输出
re_file = './256点/y_fft256_re.txt';
im_file = './256点/y_fft256_im.txt';
...
tline = fgets(fid_re);
while ischar(tline)
    y_fft256_re = [y_fft256_re; typecast(uint16(hex2dec(tline(1:4))), 'int16')];
    tline = fgets(fid_re);
end
fclose(fid_re);
...
tline = fgets(fid_im);
while ischar(tline)
    y_fft256_im = [y_fft256_im; typecast(uint16(hex2dec(tline(1:4))), 'int16')];
    tline = fgets(fid_im);
end
fclose(fid_im);

% 合并实部和虚部为复数数组
y_fft = complex(double(y_fft256_re) / (2^7), double(y_fft256_im) / (2^7));

subplot(2, 3, 3); 
plot(abs(y_fft), 'LineWidth', 1.5);
title('定点化RTL 256点FFT结果（幅度）');
xlabel('频率索引');
ylabel('幅度');
grid on;
...
% 保存标准IFFT的定点化结果
fd_re = fopen("./256点/x_ifft256_re.txt", 'wb');
fd_im = fopen("./256点/x_ifft256_im.txt", 'wb');
for i = 1:N
...
X_sim_ifft = conj(fft_radix2(X_16bit_ifft, N)) / (2^8); 
...
fd_re = fopen("./256点/x_sim_ifft256_re.txt", 'wb');
fd_im = fopen("./256点/x_sim_ifft256_im.txt", 'wb');
for i = 1:N
...
%%
% 读取RTL的输出
re_file = './256点/y_ifft256_re.txt';
im_file = './256点/y_ifft256_im.txt';
...
tline = fgets(fid_re);
while ischar(tline)
    y_ifft256_re = [y_ifft256_re; typecast(uint16(hex2dec(tline(1:4))), 'int16')];
    tline = fgets(fid_re);
end
fclose(fid_re);
...
tline = fgets(fid_im);
while ischar(tline)
    y_ifft256_im = [y_ifft256_im; typecast(uint16(hex2dec(tline(1:4))), 'int16')];
    tline = fgets(fid_im);
end
fclose(fid_im);

% 合并实部和虚部为复数数组
y_ifft = complex(double(y_ifft256_re) / (2^7), double(y_ifft256_im) / (2^7));

subplot(2, 3, 6); 
plot(abs(y_ifft), 'LineWidth', 1.5);
title('定点化RTL 256点IFFT结果（幅度）');
xlabel('频率索引');
ylabel('幅度');
grid on;

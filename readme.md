...
以下图为例，8位fft共有3层，红框框出的蝶形其层数`m=1`，组数`i=3`，组内编号`j=1`。其上节点编号 $(3-1)*2^1+ 1 = 5$ ，即为从上往下数第5个节点（因为输入已经进行了重排，所以这里的编号都是顺序的），下编号节点编号 $5+2^{1-1}=6$ ，旋转因子编号 $2^{3-1}*(1-1)=0$ ，所以旋转因子是 $W_N^0$ 。
